...
@objc class BipeLiveActivityManager: NSObject {
    ...
    static func sendBipeConfirmationAsync(execucaoId: String?) async {
        ...
        var iconToUse: String? = nil
        var insecureRegion: Bool = false
        ...
        moc.performAndWait {
            ...
            let deviceIcon = Settings.string(forKey: "icon", inMOC: moc) ?? Settings.string(forKey: "face_preference", inMOC: moc)
            iconToUse = (deviceIcon != nil && !deviceIcon!.isEmpty) ? deviceIcon : "logo"
            insecureRegion = BipeLiveActivityManager.getCurrentRegionInsecure()
            currentRegion = BipeLiveActivityManager.getCurrentRegionName()
        }
        
        guard let devId = deviceId, !devId.isEmpty else {
            NSLog("[BipeLiveActivityManager] devId não encontrado para enviar confirmação async")
            return
        }
        
        let bearerToken = await withCheckedContinuation { continuation in
            AuthManager.shared.getBearerToken { token in
                continuation.resume(returning: token)
            }
        }
        
        let endpoint = "https://dev.simodapp.com:2087/bipe/devices/\(devId)/bipe"
        ...
        do {
            ...
            if let httpResp = response as? HTTPURLResponse {
                NSLog("[BipeLiveActivityManager] HTTP ConfirmBipe OK. Status: %d", httpResp.statusCode)
            }
        } catch {
            NSLog("[BipeLiveActivityManager] Erro HTTP ConfirmBipe: %@", error.localizedDescription)
        }
        
        // Atualiza a Live Activity localmente após confirmar o Bipe
