...
@objc class SetupService: NSObject {
    ...
    private func fetchFCMToken(completion: @escaping (String?) -> Void) {
        Messaging.messaging().token { token, error in
            if let error = error {
                NSLog("[SetupService] Erro ao obter token FCM: %@", error.localizedDescription)
                completion(nil)
            } else {
                NSLog("[SetupService] Token FCM obtido: %@", token ?? "nil")
                completion(token)
            }
        }
    }
    ...
    private func persistSetup(dto: DeviceSetupResponseDto, context: NSManagedObjectContext) {
        context.performAndWait {
            ...
            // Modo MQTT (CONNECTION_MODE_MQTT = 0, definido em Settings.h)
